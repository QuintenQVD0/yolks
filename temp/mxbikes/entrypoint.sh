...
mkdir -p /home/container/screenshots
# Screenshot loop
screenshot_loop() {
    i=0
    while true; do
        xwd -root -display :0 | convert xwd:- screenshots/screenshot_$(printf "%06d" "$i").png
        i=$((i + 1))
        sleep 0.3
    done
}
...
trap "kill $SCREENSHOT_PID 2>/dev/null" EXIT SIGTERM SIGINT
