...
if [ -d "/home/container/rootfs/" ] || [ -d "${FEX_ROOTFS_PATH}/RootFS" ]; then
    ...
    FEXRootFSFetcher -y -x --distro-name=ubuntu --distro-version=20.04
fi

# Generate Config.json if needed
if [ -f "/home/container/rootfs/Ubuntu_20_04/break_chroot.sh" ] || \
   [ -f "${FEX_ROOTFS_PATH}/RootFS/Ubuntu_20_04/break_chroot.sh" ]; then
